...
{
  uint16 header_size = (uint16)*ata_read(sector_offset, 2);

  ASSERT(header_size > 0);

  uint8 *header_buffer = ata_read(sector_offset, header_size * 512) + 2;

  return epicfs_parse(header_buffer);
}

fs_entry *epicfs_parse(uint8 *buffer)
{
...
int epicfs_read_file(char *path, uint8 *buffer)
{
...
void epicfs_pretty_print_file_entry(fs_entry entry, char *path, int indent)
{
  ...
  uint32 file_size = entry.file->header.file_size;
