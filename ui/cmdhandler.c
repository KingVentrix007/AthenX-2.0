...
void parse_command(const char* command, char programs[MAX_PROGRAMS][20]) {
   ...
   while (token != NULL) {
    ...
    else if(strcmp(arguments[0],"edit") == 0)
    {
        ...
        if(index != -1 && strcmp(sub_args[0],"--help") != 0)
        {
...
//         set_scroll_mode(0);
//         if (arg_count >= 2) {
//             // Load shell first and then run the specified command
//             char* shell_args[] = {"Tristan", "/sys/shell", NULL};
//             load_elf_file(arguments[1], 2, shell_args);
            
//         } else if(arg_count == 1) {
//             printf("load shell");
//             char* shell_args[] = {"Tristan", "/sys/shell", NULL};
//             load_elf_file("/sys/shell", 2, shell_args);
...
//         clear_screen();
//     } else if (strcmp(arguments[0], "ls") == 0) {
//         // Implement the ls command to list files and directories
//         Entry f[100];
//         Entry d[100];
//         int d_int = 0;
//         int f_int = 0;
...
//         else
//         {
//              printf("Command '%s' is deprecated and/or non existant. Please load the your shell or the default shell and then run the command.\n", arguments[0]);
//         }
